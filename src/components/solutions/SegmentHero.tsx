...
export function SegmentHero({ current }: SegmentHeroProps) {
  ...
  const heroTitle = t(`segments.${current.slug}.heroTitle`, { defaultValue: current.heroTitle });
  const heroDescription = t(`segments.${current.slug}.heroDescription`, { defaultValue: current.heroDescription });
  return (
      ...
      <ScrollReveal>
        <div className="max-w-6xl mx-auto text-center space-y-10">
          ...
          <Button size="lg" className="px-12 py-6 text-base btn-glow" asChild>
            <Link to="/open-account">{t("segmentHero.register")}</Link>
          </Button>
        </div>
      </ScrollReveal>
    </section>
